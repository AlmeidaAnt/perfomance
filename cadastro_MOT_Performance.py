...
# 1. Acessar a aba de notificações
...
for coluna in planilha.iter_rows(min_row=2, values_only=True):
    ...
    cadastro_novo = navegador.find_element(By.XPATH,'/html/body/div[2]/div/div/div[1]/div[2]/button[3]')
    cadastro_novo.click()
    time.sleep(1)
    nome = navegador.find_element(By.XPATH,'//*[@id="name"]')
    nome.send_keys(motorista)
    time.sleep(1)
    classificacao = navegador.find_element(By.XPATH,'//*[@id="driver"]')
    classificacao.click()
    time.sleep(1)
    editar_matricula = navegador.find_element(By.XPATH,'//*[@id="registration"]')
    editar_matricula.send_keys(matricula)
    time.sleep(1)
    editar_chaveiro = navegador.find_element(By.XPATH,'//*[@id="cardId"]')
    editar_chaveiro.send_keys(chaveiro)
    time.sleep(1)
    salvar = navegador.find_element(By.XPATH,'/html/body/div[2]/div/div[1]/button')
    salvar.click(), Keys.ENTER
    time.sleep(1)
